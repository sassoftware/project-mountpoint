...
> Note this exercise specifies Rancher local-path and a custom daemonset to format and mount the node-local SSD disk for SASWORK and CAS_DISK_CACHE.
...
1.  Add custom labels we can use later to identify instances with local disk

    ```bash
    # Insert the custom label for CAS hosts
    sed -i '/"workload.sas.com\/class" = "cas"/a\ \ \ \ \ \ "attr.sas.com/local-nvme" = "true"' gel-vars.tfvars

    # Insert the custom label for SPRE hosts
    sed -i '/"launcher.sas.com\/prepullImage" = "sas-programming-environment"/a\ \ \ \ \ \ "attr.sas.com/local-nvme" = "true"' gel-vars.tfvars
...
1.  Apply the Terraform plan:
    ...
    It only takes 5-10 minutes for Terraform to complete the provisioning of resources in Azure.
...
1.  Format and mount additional local disk drives
    ...
    > ALREADY DONE. AKS does this for us automatically. The extra node-local disk is mounted at `/mnt` by default. We will configure the local-path-provisioner to use it.
    >
    > ```log
    > NAME    MOUNTPOINT   SIZE TYPE FSTYPE
    > sda                  200G disk 
    > |-sda1  /          199.9G part ext4
    > |-sda14                4M part 
    > `-sda15 /boot/efi    106M part vfat
    > sdb                  300G disk 
    > `-sdb1  /mnt         300G part ext4
    > sr0                  760K rom
    > ```
    >
    > FYI:
    > - `/` (root volume): located on `sda1` (199G partition on 200G disk)
    > - `/mnt` (addtl volume): located on `sdb1` (300G partition on 300G disk)
...
1. We need a custom path to use `/viya-scratch`. Modify the configMap:
    ...
    Replace the default path "`/opt/local-path-provisioner`" with our new mounted volume "`/mnt`". And save the file.
    ...
    | jq '.data["config.json"] |= (fromjson | .nodePathMap[0].paths = ["/mnt/viya-scratch"] | tojson)' \
    | kubectl apply -f -
    ```
...
NAME                    PROVISIONER             RECLAIMPOLICY   VOLUMEBINDINGMODE      ALLOWVOLUMEEXPANSION   AGE
azurefile               file.csi.azure.com      Delete          Immediate              true                   137m
azurefile-csi           file.csi.azure.com      Delete          Immediate              true                   137m
azurefile-csi-premium   file.csi.azure.com      Delete          Immediate              true                   137m
azurefile-premium       file.csi.azure.com      Delete          Immediate              true                   137m

default                 disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   137m
managed                 disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   137m
managed-csi             disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   137m
managed-csi-premium     disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   137m
managed-premium         disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   137m

local-path              rancher.io/local-path   Delete          WaitForFirstConsumer   false                  31m
nfs                     nfs.csi.k8s.io          Retain          WaitForFirstConsumer   false                  2m36s

viya-scratch-sc         rancher.io/local-path   Delete          WaitForFirstConsumer   false                  29m
viya-shared-sc          nfs.csi.k8s.io          Retain          WaitForFirstConsumer   false                  11s
viya-standard-sc        disk.csi.azure.com      Delete          WaitForFirstConsumer   true                   39m
```

> Note: this approach allows us to specify storage for Viya ***without modifying Viya's storage configuration***. Don't want "`local-path`" as the basis for "`viya-scratch-sc`"? Then use "`managed`" instead.
