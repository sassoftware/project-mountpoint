...
1.  Download the viya4-iac-gcp project from `github.com/sassoftware` to get the Terraform templates that SAS maintains to provision infrastructure in the Google Cloud.
    ...
    # Instead of being at the mercy of the latest changes, we can pin to a specific version
    #IAC_GCP_TAG=v7.11.0
...
1.  Update kubectl to match the Kubernetes server version
    ...
    export GKE_FULL_VER=$(gcloud container clusters describe ${MY_PREFIX}-gke --region=$MY_GCZONE --format="value(currentMasterVersion)" | cut -d'-' -f1) && echo -e "Kubernetes version = $GKE_FULL_VER\n\n"
    ```

    > Note that the `--region` parameter is populated with the value from `$MY_GCZONE` ("us-east1-b") instead of the value from `$MY_GCREGION` ("us-east1") -- because it recently wanted it that way. :-\
