...
create_launcher_context() {
    ...
    sas-viya launcher contexts create \
        ...
        --job-pod-template-name "${pod_template}" 2>&1 | tee "${WORK_DIR}/launcher_${pod_template}.log"
}
        ...
        compute|batch)
            ...
            case "${CONTEXT_TYPE}" in
                compute)
                    ...
                    LAUNCHER_CONTEXT_NAME=$(jq -r '.launchContext.contextName' "${CONTEXT_JSON}")
                    ;;
                batch)
                    ...
                    # Get launcher context ID and then look up the name
                    local LAUNCHER_CONTEXT_ID=$(jq -r '.launcherContextId' "${CONTEXT_JSON}")
                    if [[ -z "${LAUNCHER_CONTEXT_ID}" ]] || [[ "${LAUNCHER_CONTEXT_ID}" == "null" ]]; then
                        echo "ERROR: Could not find launcher context ID for batch context '${CONTEXT_NAME}'"
                        return 1
                    fi
                    
                    # Get launcher context name from ID
                    # Get launcher context name from ID by querying directly
                    LAUNCHER_CONTEXT_NAME=$(sas-viya --output fulljson launcher contexts show --id "${LAUNCHER_CONTEXT_ID}" | jq -r '.name')
                    ;;
            esac

            if [[ -z "${LAUNCHER_CONTEXT_NAME}" ]] || [[ "${LAUNCHER_CONTEXT_NAME}" == "null" ]]; then
