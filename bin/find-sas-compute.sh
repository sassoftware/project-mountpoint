...
# Script to find the newest sas-compute-server pod and locate its storage paths (viya PVC & emptydir)
# Usage: ./find-sas-compute.sh

set -e

echo "🔍 Finding newest sas-compute-server pod..."

# Find all sas-compute-server pods and get the newest one with proper output format
NEWEST_POD_INFO=$(kubectl get pods -A -o wide --no-headers --sort-by=.metadata.creationTimestamp | grep sas-compute-server | tail -1)


if [ -z "$NEWEST_POD_INFO" ]; then
    echo "❌ No sas-compute-server pods found!"
    exit 1
fi
...
echo "   namespace: $NAMESPACE, node: $NODE_NAME"

# Get PVC information - find any PVC associated with this pod
...
if [ -n "$PVC_INFO" ]; then
    ...
    # Get the parent directory since /tmp and /viya are subdirectories
...
if [ -n "$LOCAL_PATH_ROOT" ]; then
    echo "# Local-path volumes (contains /tmp and /viya subdirectories):"
