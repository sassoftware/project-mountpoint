...
################################################################################
# This script attaches the AmazonEBSCSIDriverPolicy to the IAM role associated
...
# with the instance profile and attach the AmazonEBSCSIDriverPolicy to the role.
#
...
CLUSTER_NAME="$(kubectl config view -o jsonpath='{.contexts[0].name}')"
...
for INSTANCE_ID in $WORKER_INSTANCE_IDS; do
  ...
  #IAM_ROLE_NAME=$(aws iam list-roles --query "Roles[?InstanceProfileList[?Id=='$INSTANCE_PROFILE_INFO']].RoleName" --output text 2>/dev/null)
  #IAM_ROLE_NAME=$(aws iam list-roles --query "Roles[?InstanceProfileList[?Id=='$INSTANCE_PROFILE_INFO']].RoleName" --output text 2>/dev/null)
  ...
  POLICY_ATTACHED=$(aws iam list-attached-role-policies --role-name "$IAM_ROLE_NAME" --query "AttachedPolicies[?PolicyArn=='$EBS_CSI_POLICY_ARN'].PolicyArn" --output text 2>/dev/null)

  if [ -z "$POLICY_ATTACHED" ]; then
    echo "  Attaching AmazonEBSCSIDriverPolicy to role '$IAM_ROLE_NAME'..."
    echo "  >> aws iam attach-role-policy --role-name \"$IAM_ROLE_NAME\" --policy-arn \"$EBS_CSI_POLICY_ARN\"" 
    aws iam attach-role-policy --role-name "$IAM_ROLE_NAME" --policy-arn "$EBS_CSI_POLICY_ARN" 2>/dev/null
    if [ $? -eq 0 ]; then
      echo "  Successfully attached AmazonEBSCSIDriverPolicy to role '$IAM_ROLE_NAME'."
    else
      echo "  Error attaching AmazonEBSCSIDriverPolicy to role '$IAM_ROLE_NAME'."
    fi
  else
    echo "  AmazonEBSCSIDriverPolicy is already attached to role '$IAM_ROLE_NAME'."
  fi
done

echo "Finished attaching AmazonEBSCSIDriverPolicy to k8s worker nodes in cluster '$CLUSTER_NAME'."
